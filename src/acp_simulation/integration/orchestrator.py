...
class CombinatorialTestingOrchestrator:
    ...
    def __init__(
        self, acts_jar_path: str, ccm_jar_path: str, output_dir: str = "./combinatorial_results"
    ):
        """
    ...
    def run_full_workflow(
        ...
        if execute_tests:
            print(
                f"Test results: {results['test_execution']['successful']}/{results['test_execution']['num_tests']} successful"
            )

        if analyze_coverage:
            print(
                f"Coverage: See {results['coverage_analysis'].get('report_file', 'coverage report')}"
            )
...
def run_combinatorial_validation(
    ...
    orchestrator = CombinatorialTestingOrchestrator(
        acts_jar_path=acts_jar_path, ccm_jar_path=ccm_jar_path, output_dir=output_dir
    )
