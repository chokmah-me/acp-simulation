...
class TestACTSGenerator:
    ...
    def test_create_acts_input_basic(self):
        ...
        parameters = [ACTSParameter("test_param", "int", [1, 2, 3])]

        input_content = generator._create_acts_input(
            parameters=parameters, constraints=None, strength=2, algorithm="ipog"
        )
    ...
    def test_create_acts_input_with_constraints(self):
        ...
        constraints = [ACTSConstraint("param1 > param2")]

        input_content = generator._create_acts_input(
            parameters=parameters, constraints=constraints, strength=2, algorithm="ipog"
        )
    ...
    def test_create_acts_input_format(self):
        ...
        input_content = generator._create_acts_input(
            parameters=parameters, constraints=None, strength=3, algorithm="ipog_d"
        )
...
class TestACTSGeneratorIntegration:
    ...
    def test_generate_covering_array_2_way(self, acts_jar_path):
        ...
        covering_array = generator.generate_covering_array(parameters=parameters, strength=2)
    ...
    def test_generate_covering_array_with_acp_parameters(self, acts_jar_path):
        ...
        assert all(
            col in covering_array.columns for col in ["acp_strength", "num_nodes", "connectivity"]
        )
    ...
    def test_generate_covering_array_with_constraints(self, acts_jar_path):
        ...
        constraints = [ACTSConstraint("(num_nodes = 500) => (num_episodes <= 5000)")]

        covering_array = generator.generate_covering_array(
            parameters=parameters, constraints=constraints, strength=2
        )
        ...
        violating_rows = covering_array[
            (covering_array["num_nodes"] == 500) & (covering_array["num_episodes"] > 5000)
        ]
...
class TestACTSGeneratorOutput:
    ...
    def test_algorithm_parameter(self):
        ...
        input_ipog = generator._create_acts_input(parameters, None, 2, "ipog")
        assert "[System]" in input_ipog

        # Test with ipog_d algorithm
        input_ipog_d = generator._create_acts_input(parameters, None, 2, "ipog_d")
